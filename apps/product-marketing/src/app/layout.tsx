...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  // Optional: the homepage's embedded canvasser demo derives field.<host> when this is unset.
      ...
      <body className={`${outfit.variable} bg-gray-50`}>
        <script
          dangerouslySetInnerHTML={{
            __html:
              `window.__API_URL__=${JSON.stringify(apiUrl)};window.__AUTH_APP_URL__=${JSON.stringify(authAppUrl)};window.__APP_URL__=${JSON.stringify(appUrl)};window.__TURNSTILE_SITE_KEY__=${JSON.stringify(turnstileSiteKey)};` +
              (fieldAppUrl ? `window.__FIELD_APP_URL__=${JSON.stringify(fieldAppUrl)};` : ""),
          }}
        />
        <SessionProvider>
