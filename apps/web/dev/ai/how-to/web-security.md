...
Canonical: `apps/auth` (the standalone identity app – routes `/login`, `/sign-up`, `/magic-link`, `/verify-email`, `/reset-password`, `/2fa`, `/invite/[token]`, `/select-tenant`), `apps/web/src/middleware.ts` (the SSO gate), `apps/api/src/auth/session-cookie.util.ts` (`SESSION_COOKIE = "auth_token"`, `sessionCookieOptions`, `SESSION_COOKIE_DOMAIN`), `apps/web/src/lib/session.ts` (`getSession`, `goToLogin`, `logout`), `packages/api-client/src/index.ts` (`request` with `credentials: "include"`; its internal `redirectToLogin` bounces to the auth app on 401), `docs/meld/14-auth-frontend-and-sso.md`.

## Must have
- The session is the `auth_token` httpOnly cookie issued by `apps/api`, scoped to the parent domain via `SESSION_COOKIE_DOMAIN` (e.g. `.yarns.org.au` in prod, `.lvh.me` in dev) so every subdomain app shares one session. The cookie is `httpOnly`, `sameSite: "lax"`, `secure` in production. Frontends never read or write it directly.
- Send credentials by cookie, not header: API calls go through `@yarns/api-client`'s `request`, which sets `credentials: "include"`. The API CORS must allow that origin with credentials.
- `apps/web/src/middleware.ts` gates routes: no `auth_token` cookie → redirect to `auth.<domain>/login?return_to=<self>`. Keep the matcher excluding `_next`, service-worker assets and static files so the login bounce never blocks JS/CSS.
...
- Credential entry, token issuance, invites, magic-link landing, 2FA and tenant selection live only in `apps/auth`. Other apps redirect to it – never add a `/login` form or hold credentials in `apps/web`.
...
- A bespoke login form or stored credentials in `apps/web` – the old localStorage Basic-creds path was removed; all of it lives in `apps/auth`.
...
- [ ] API calls use the cookie via `@yarns/api-client` (no manual auth header in the browser).
- [ ] No credential entry / secret added to `apps/web`; auth flows stay in `apps/auth`.
- [ ] No secret reaches a client component or `NEXT_PUBLIC_*`.
- [ ] Gate: walk `dev/ai/how-to/definition-of-done.md`.

## Related guides
- `apps/web/dev/ai/how-to/app-router-and-api-client.md` – the client wrapper that carries the cookie.
- `apps/web/dev/ai/how-to/permission-gating.md` – authorising the authenticated principal.
