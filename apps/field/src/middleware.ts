...
export const config = {
  ...
  // target and the status page's health probe (both unauthenticated by design — must
  // reach the lambda, not bounce at the edge)
  ...
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|manifest.webmanifest|sw.js|workbox-|worker-|fallback-|images/|icons/|api/warm|api/health|demo(?:$|/)|.*\\.(?:png|jpg|jpeg|svg|gif|ico|json|js|css|woff2?)).*)",
  ],
};
