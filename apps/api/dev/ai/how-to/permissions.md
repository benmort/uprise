---
name: permissions
description: How to authorise an endpoint in uprise – auth is enforced on every route, but @RequirePermission gating is opt-in, so always decorate.
layer: api
topic: auth
...
- Use a real `action` (`read`/`create`/`update`/`delete`/`operate`/`manage` from `STANDARD_ACTIONS`) and a namespaced `<domain>.<entity>` resource from `UPRISE_RESOURCES` (e.g. `{ action: "manage", resource: "messaging.blast" }`).
...
- A new resource string must be added to `UPRISE_RESOURCES` in `packages/permissions/src/types.ts` and granted in `ROLE_PERMISSIONS` (`packages/permissions/src/roles.ts`) – rebuild `@uprise/permissions` after.
...
- Inventing a resource string not in `UPRISE_RESOURCES` – the matrix never grants it, so it silently 403s for everyone except super-admin (`manage all`).
...
## Checklist
- [ ] Endpoint carries `@RequirePermission` with an action + a resource that exists in `UPRISE_RESOURCES` (no route left undecorated by oversight).
- [ ] The role(s) that should reach it are granted in `ROLE_PERMISSIONS`; `@uprise/permissions` rebuilt if you touched it.
