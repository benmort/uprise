...
// (/prog/shared-inbox/<prefix>/<uid>, e.g. /e/priya for email, /s/deb for SMS) so a
// loaded resource has a dedicated, shareable URL. Wiring this to the real inbox API
// (keyed by the same channel + uid) is the next step.
 ...
 *  /prog/shared-inbox/<folder>/<prefix>/<uid>. */
export function conversationHref(
  c: Pick<Conversation, 'channel' | 'uid'>,
  folder: string = DEFAULT_FOLDER,
): string {
  return `/prog/shared-inbox/${folder}/${CHANNELS[c.channel].prefix}/${c.uid}`;
}
