 ...
 * (deep-linkable, survives reload) and is read by the surface's map to paint the
 * picked state's boundary. Universe comes from `useGeoExplorer()`. States are a
 * small fixed set, so search is a client filter with no sub-tabs or pagination.
 */
export function StatesPanel({ view }: { view: WalkMode }) {
  const { q, state } = useGeoExplorerUrlState({ viewStorageKey: "uprise.statesView" });
  const { universe, setUniverse } = useGeoExplorer();
