...
export interface FormInputWithStatusProps extends Omit<React.ComponentProps<'input'>, 'className'> {
  label: string;
  id?: string;
...
export function FormInputWithStatus({
  label,
  id,
  status,
  message,
  className,
  ...props
}: FormInputWithStatusProps) {
  const inputId = id ?? `input-status-${Math.random().toString(36).slice(2)}`;

  const stateClasses =
    status === 'error'
      ? 'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800 pr-10'
      : 'border-success-300 focus:border-success-300 focus:ring-success-500/10 dark:border-success-700 dark:focus:border-success-800 pr-10';

  const messageClasses =
    status === 'error' ? 'text-theme-xs text-error-500' : 'text-theme-xs text-success-500';
  ...
  return (
    <FormField label={label} htmlFor={inputId}>
      <div className="relative">
        <input
          id={inputId}
          className={cn(inputBaseClasses, stateClasses, className)}
          {...props}
        />
      ...
      <p className={cn('mt-1.5', messageClasses)}>{message}</p>
    </FormField>
  );
}
