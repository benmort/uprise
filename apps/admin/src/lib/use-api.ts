...
/**
 * The admin app's one data-fetching primitive – thin React binding over the
 * use-api-cache engine (TTL cache, in-flight dedup, stale-while-revalidate,
 * refcounted abort). Semantics matched to what the hand-rolled pages need:
 *
 ...
 * - errors keep the last-good data (pages can show both);
 * - `refetchInterval` (number | false) polls, paused while the tab is hidden,
 *   with an immediate revalidate when it becomes visible again;
 * - StrictMode-safe: reads via useSyncExternalStore, fetches only in effects.
 *
 * Keys are request paths by convention (e.g. "/geo/divisions?type=ced") so
 * `invalidateApi("/files")` after a mutation maps onto the REST resource.
 */
