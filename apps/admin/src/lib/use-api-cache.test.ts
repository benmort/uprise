...
describe("use-api cache engine", () => {
  ...
  it("invalidateApi(prefix) marks matching keys stale and refetches subscribed ones", async () => {
    ...
    let version = 1;
    entryFor("/files?take=50").fetcher = async () => ok(`v${version}`);
    subscribeKey("/files?take=50", () => {});
    await revalidate("/files?take=50");
    expect(peekEntry("/files?take=50")?.data).toBe("v1");

    // Unsubscribed key just goes stale.
    await revalidate("/geo/divisions", async () => ok("divisions"));

    version = 2;
    invalidateApi("/files");
    await new Promise((r) => setTimeout(r, 5));
    expect(peekEntry("/files?take=50")?.data).toBe("v2"); // refetched
    expect(isFresh("/geo/divisions", 60_000)).toBe(true); // untouched prefix
  });
