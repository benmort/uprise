 ...
 * React binding so its semantics are unit-testable in node (the admin vitest
 * env has no DOM).
 */
...
/** Soft cap on cached keys. Boundary-detail keys can hold multi-MB GeoJSON, so
 ...
 * request paths, so `invalidateApi("/files")` maps onto the REST resource).
