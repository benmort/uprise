...
- Route-gate whole shells by role: the `(main)` layout bounces a `CANVASSER` to `/field` (`session.role === "CANVASSER"`). This is defence-in-depth for UX – the server still authorises each mutation via its own `@RequirePermission` decorator.
