...
description: How yarns frontends fetch – through the typed @yarns/api-client – and how to respect Next App Router server/client boundaries.
layer: web
topic: data-fetching
...
- Type request/response with the shared `@yarns/contracts` types re-exported from `@yarns/api-client` (e.g. `AuthPrincipal`, `CheckSessionResponse`). Keep the contract the single shape both ends agree on.
...
- Restating an API DTO as a local `interface` instead of importing it from `@yarns/contracts`.
...
## Checklist
- [ ] All API traffic goes via `@yarns/api-client` / a `lib/api` helper, not bare `fetch`.
- [ ] Types come from `@yarns/contracts`; the `ApiResult.ok` branch is handled.
