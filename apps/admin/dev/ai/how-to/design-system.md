---
name: design-system
description: How to style uprise frontends – @uprise/ui tokens and primitives are the single source of truth.
layer: web
topic: styling
...
Every uprise frontend renders from one shared design system – `@uprise/ui` – consumed as source, never re-themed ad hoc.
...
## Must have
- Treat `@uprise/ui` as the source of truth. Import primitives via the app shim `@/components/ui/*` (which re-exports `@uprise/ui`) – `Button`, `StatusBadge`, `EmptyState`, `Skeleton`, `Field`, `FormDialog`, etc. Do not hand-roll a primitive that already exists.
- Consume the package from source – no build step. `apps/admin/next.config.mjs` lists `@uprise/ui` in `transpilePackages`; editing `packages/ui/src` is live in dev.
- An app imports the tokens once: `@import "tailwindcss";` then `@import "@uprise/ui/globals.css";` in its own `globals.css`, plus an `@source` line pointing at `packages/ui/src/**/*.{ts,tsx}` so Tailwind v4 scans the shared classes.
...
- Forking a UI primitive into `apps/admin` instead of fixing or extending `@uprise/ui`.
...
- Building a `dist` for `@uprise/ui` and importing that – it ships TS/TSX source via `transpilePackages`.

## Checklist
- [ ] Reused a `@uprise/ui` primitive (or extended the package), not a local fork.
