---
name: web-engineer
description: Frontend layer-engineer entry point for the yarns web apps (web/auth/marketing) – the invariants every UI change obeys plus a router to every frontend how-to. Use when building or changing any page, component, route, form, settings surface, or auth-touching frontend in apps/admin, apps/auth or apps/marketing.
---
...
- **Typed API only.** All API traffic goes through `@yarns/api-client`'s `request<T>` (or a `lib/api` helper that wraps it) – never a bare `fetch` to the API. Type with `@yarns/contracts`; branch on the `ApiResult.ok` union before reading `data` or `error`. Keep `"use client"` to the interactive island, not the whole page. See `apps/admin/dev/ai/how-to/app-router-and-api-client.md`.
...
4. **Build from primitives.** Reuse `@yarns/ui`; if a primitive is missing, extend the package, don't fork it into `apps/admin`. Tokens only, no hex.
...
- Raw hex/rgb in a className (`text-[#b45309]`) instead of a token (`text-warning-foreground`); forking a `@yarns/ui` primitive into `apps/admin`.
