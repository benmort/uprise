...
- **Two targets, and they are not equivalent.**
  ...
  A local `.env` whose `CORS_ALLOWED_ORIGINS` omits `http://localhost:3002` / `:3000` will make every auth-app→API call fail with "Failed to fetch" on the localhost target. If a spec needs the browser to reach the API, use the ngrok target or add those origins. On the admin app this failure wears a disguise: every page test dies on the **"We couldn't reach the server – you're still signed in, we just couldn't confirm it"** screen (the layout's session-check saw a CORS-blocked fetch, not a 401) while API-context tests in the same run pass. That split – browser tests all failing, `request`-only tests green – is the CORS signature; re-run with `E2E_TARGET=ngrok` before debugging anything else.
