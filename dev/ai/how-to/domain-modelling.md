...
description: How to model a new backend domain in yarns – schema namespace, id-only boundaries, events, and FSM – before cutting code.
layer: root
topic: process
...
How to shape a new yarns domain before writing it. yarns is a modular monolith: each domain is a Prisma schema namespace + a Nest module under `apps/api/src/<domain>/`.
...
- **Decide the events.** What other domains need to know about → add typed entries to `@yarns/events` (`packages/dev/ai/how-to/events-catalogue.md`) and emit them via the outbox on the state writes.
...
- [ ] Events added to `@yarns/events`; emitted via the outbox.
