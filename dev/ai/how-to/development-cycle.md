...
description: How a unit of work moves from request to merged in yarns – work shapes, gates, and hand-off. Board-free.
layer: root
topic: process
...
How work moves from request to merged. yarns has no story board – the unit of work is the task in front of you, tracked with the task tool and the plan file, not a tracker.
...
- **Multi-layer / large** – plan first (plan tool), break into milestones, validate after each. Optionally fan out via `yarns-implement` / `yarns-dispatch`.
...
3. **Execute** – follow the guide's Must-have + Checklist. Keep diffs minimal. Rebuild any `@yarns/*` dist you touched.
