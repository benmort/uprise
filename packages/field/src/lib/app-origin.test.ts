...
/**
 * The window-bound wrapper.
 *
 * This suite was written when the package's vitest environment was `node`, so "no window" was
 * simply the default and the SSR case needed no setup. The environment is jsdom now (component
 * and hook tests need a DOM), so jsdom always supplies a `window` and the SSR branch has to be
 * simulated explicitly — see withoutWindow.
 */
describe("adminOrigin", () => {
  /** Run `fn` with NO `window` at all — the server render. */
  function withoutWindow<T>(fn: () => T): T {
    const g = globalThis as unknown as { window?: unknown };
    const had = "window" in g;
    const prev = g.window;
    // @ts-expect-error deliberately removing the global to stand in for SSR
    delete g.window;
    try {
      return fn();
    } finally {
  ...
  it("returns empty during SSR, where there is no host to derive from", () => {
    ...
    expect(withoutWindow(adminOrigin)).toBe("");
  });
