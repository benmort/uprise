...
description: How to add and consume a typed domain event in @yarns/events.
layer: packages
topic: events
...
`@yarns/events` is the single typed contract for the outbox/reactions backbone – every emitted event and every `Reaction` is generic over it.
...
- After editing `src/index.ts`, rebuild the package: `pnpm --filter @yarns/events build`. The api and worker import the built `dist`, so an unbuilt change is invisible to them.
...
- [ ] `pnpm --filter @yarns/events build` run so api/worker pick it up.
