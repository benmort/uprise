...
description: The action/resource/role taxonomy in @yarns/permissions and how routes reference it.
layer: packages
topic: authorisation
...
`@yarns/permissions` is the single source of the authorisation matrix – the action and resource taxonomy plus the role → permissions table the api's guard builds an ability from.

Canonical: `packages/permissions/src/types.ts` (`STANDARD_ACTIONS`, `YARNS_RESOURCES`, `PermissionRule`, `AuthenticatedActor`) and `packages/permissions/src/roles.ts` (`YARNS_ROLES`, `ROLE_PERMISSIONS`, `APP_USER_ROLE_TO_ROLE`).
...
- A new resource must be added to the `YARNS_RESOURCES` array before it can be referenced; same for a new action and `STANDARD_ACTIONS`.
- Grant a role access by adding a `PermissionRule` (`{ action, resource }`) to its entry in `ROLE_PERMISSIONS`. Role rules are always positive – no `inverted`. Roles are the five in `YARNS_ROLES` (`super-admin`, `owner`, `organiser`, `canvasser`, `member`); the legacy Prisma `AppUserRole` maps via `APP_USER_ROLE_TO_ROLE`.
...
- After editing `src/`, rebuild: `pnpm --filter @yarns/permissions build` – the api imports the built `dist`.

## Anti-patterns
- Referencing a resource/action string not in `YARNS_RESOURCES`/`STANDARD_ACTIONS` – it is untyped and silently never matches a role rule.
...
## Checklist
- [ ] New resource added to `YARNS_RESOURCES` (namespaced `<domain>.<entity>`); new action added to `STANDARD_ACTIONS`.
...
- [ ] `pnpm --filter @yarns/permissions build` run.
