...
Direction-setting research for a **door-knocking (canvassing) domain in yarns,
...
[`yarns-direction.md`](./yarns-direction.md).
...
1. [`synthesis.md`](./synthesis.md) – start here. Capability matrix across all
   11 products plus the cross-product patterns for the four things yarns cares
   about (door↔inbox coupling, shared scripts, shared surveys, journeys) and a
   recommended disposition taxonomy.
2. [`yarns-direction.md`](./yarns-direction.md) – the findings mapped onto yarns'
...
- [Action Network](./products/action-network.md) – ladders of engagement (the
  journeys analogue); canonical question model; already integrates with yarns.
...
strengths/gaps, what yarns should borrow/avoid, sources). `synthesis.md` and
`yarns-direction.md` were written from the verified dossiers.
