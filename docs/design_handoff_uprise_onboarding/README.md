...
|---|---|---|
| **S1** Getting started — owner | Four flows in order Identity (required: email + mobile) → Organisation → Account (recommended extras incl. branding, chip not count) → Channels; the headline count covers Identity + Organisation only, per-flow `StepProgress` + "n of m", per-step rows with chips + CTAs, the server `reason` line on action_required (compliance rejected), the telephony card embedded with its run timeline, the email request row | `getting-started/page.tsx` rewrite: `SetupFlowSection` + `SetupStepRow` + `SetupChip` over `GET /tenants/:id/setup`; embeds the existing `<TelephonyStatusCard onboarding />` (`id="numbers"`) + new `EmailSetupCard` (`id="email"`); overall strip = `StepProgress` + copy naming the next unlock |
| **S2** Getting started — organiser | Identity + Account setup only; org/channels absent (hidden, not locked) | Same page; flows render only when `applicable` in the payload |
