...
| Dimension | Slingshot | Yarns after meld | Merge implication |
|---|---|---|---|
...
| Contracts | Zod in `packages/*` | Zod in `@yarns/contracts` (doc 01) | ✅ same pattern |
...
1. **`packages/*` workspace tier** (doc 01) – matches slingshot's contracts-in-packages layout. `@yarns/permissions`, `@yarns/events`, `@yarns/contracts` are the join points.
...
4. **BullMQ + domain-events** (doc 05) – slingshot already uses BullMQ; its per-domain queues fold into the same registry, and its processors can subscribe to yarns domain events (slingshot lacks cross-domain event contracts today – ours become the shared standard).

## The one hard divergence: MikroORM → Prisma

Slingshot's ~200 business entities use MikroORM decorators; yarns is Prisma. Bridge options for the future merge (decide then, not now):
...
- Settlement/reconciliation (Xero, FRU) has no yarns analogue – it lands as new domain schemas at merge time.

## Verification

No runtime check now. A design review at merge-planning time confirms yarns' schema-per-domain + id-only-ref + `packages/*` + CASL shape still matches slingshot's `DOMAINS` registry and contracts pattern.
