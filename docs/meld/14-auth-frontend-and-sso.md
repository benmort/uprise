...
The meld produces multiple frontends — yarns `apps/admin` (organiser `(main)` + field `(field)`), prog's `admin-client` + `marketing-client`, slingshot's `admin-ui` + `web`. A standalone identity app is the natural SSO hub for all of them, and the lowest-rework path to the merged platform:
...
- **`@yarns/ui`** — yarns' Radix/Tailwind design system, extracted to a package so `apps/auth` and `apps/admin` render identically. *(New extraction; prerequisite.)*
- **`@yarns/api-client`** — `apps/admin/src/lib/api.ts` (typed request wrapper + session handling) extracted to a package. *(New extraction; prerequisite.)*
